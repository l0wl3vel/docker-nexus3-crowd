...
RUN yum install -y git && \
    ...
    git checkout tags/nexus3-crowd-plugin-3.2.5 && \
    mvn install && \
    cp -ra ~/.m2/repository/com/pingunaut /opt/sonatype/nexus/system/com && \
    echo -e 'mvn\:com.pingunaut.nexus/nexus3-crowd-plugin/3.2.5 = 200' >> /opt/sonatype/nexus/etc/karaf/startup.properties && \
    yum remove -y git && \
